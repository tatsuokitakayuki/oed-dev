const OED_VERSION = '4.0.20200827.0';
const OED_BASE = 'OED';
const ACE_VERSION = '1.4.12-u1';
const ACE_BASE = 'Ace';
const MDC_VERSION = '7.0.0-u1';
const MDC_BASE = 'material-components-web';
const MDI_VERSION = '3.0.2-u1';
const MDI_BASE = 'material-icons';
const LF_VERSION = '1.9.0-u1';
const LF_BASE = 'localForage';
const EC_VERSION = '1.0.0-u1';
...
const MDC_URLS = [
    ...
    '/material-components-web/material-components-web.min.css.map',
...
const getCacheName =
    cacheItem => cacheItem.base + JOINT_FOR_KEY + cacheItem.version;

self.addEventListener('install', event => {
    event.waitUntil((async () => {
        const keylist = await caches.keys();
        const targets = CACHE_LIST.filter(
            item => !keylist.includes(getCacheName(item))
        );
        let promises = [];
        for (const item of targets) {
            const cache = await caches.open(getCacheName(item));
            if (cache) {
                promises.push(cache.addAll(item.list));
            }
        }
        return promises;
    })());
...
self.addEventListener('activate', event => {
    event.waitUntil((async () => {
        ...
        for (const item of CACHE_LIST) {
            keys.push(getCacheName(item));
        }
        let promises = [];
        keylist
            .filter(key => !keys.includes(key))
            .map(key => promises.push(caches.delete(key)));
        return promises;
    })());
});
